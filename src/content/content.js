...
(async function () {
  ...
  const removeExistingIndicators = () => {
    const existingBorder = document.getElementById('where-am-i-border');
    const existingTopBorder = document.getElementById('where-am-i-top-border');
    const existingBalloon = document.getElementById('where-am-i-balloon');
    const existingStyle = document.getElementById('where-am-i-styles');
    const existingBorderHover = document.getElementById('where-am-i-border-hover');
    const existingTopHover = document.getElementById('where-am-i-top-hover');
    const existingTooltip = document.getElementById('where-am-i-tooltip');
  ...
  const applyFullBorder = (color, width, name) => {
    ...
    style.textContent = `
      ...
      #where-am-i-border-hover .hover-zone {
        position: absolute;
        pointer-events: auto;
      }
      
      #where-am-i-border-hover .hover-zone.top {
        top: 0;
        left: 0;
        right: 0;
      ...
      #where-am-i-border-hover .hover-zone.bottom {
        bottom: 0;
        left: 0;
        right: 0;
        height: calc(${width} + 10px);
      ...
      #where-am-i-border-hover .hover-zone.left {
        top: 0;
        left: 0;
        bottom: 0;
      ...
      #where-am-i-border-hover .hover-zone.right {
        top: 0;
        right: 0;
        bottom: 0;
        width: calc(${width} + 10px);
    ...
    borderDiv.id = 'where-am-i-border';
  ...
  const applyTopBorder = (color, width, name) => {
    ...
    topBorderDiv.id = 'where-am-i-top-border';
    document.body.appendChild(topBorderDiv);

    // Create hover zone and tooltip
    const hoverZone = document.createElement('div');
    hoverZone.id = 'where-am-i-top-hover';
    document.body.appendChild(hoverZone);

    const tooltip = document.createElement('div');
    tooltip.id = 'where-am-i-tooltip';
  ...
  const createHoverZonesAndTooltip = (name, color) => {
    ...
    hoverContainer.id = 'where-am-i-border-hover';
  ...
  const applyBalloon = (color, name) => {
    ...
    style.textContent = `
      ...
      #where-am-i-balloon:hover {
        opacity: 1;
        transform: scale(1.02);
      }
      
      #where-am-i-balloon::before {
        content: '';
        width: 8px;
        height: 8px;
        background: rgba(255,255,255,0.9);
        border-radius: 50%;
        animation: where-am-i-pulse 2s infinite;
      }
      
      @keyframes where-am-i-pulse {
        0%, 100% { opacity: 1; }
        50% { opacity: 0.5; }
      }
      
      #where-am-i-balloon.minimized {
        padding: 6px 10px;
        font-size: 10px;
      }
    `;
